...
pub fn list_resource_packs(instance_dir: String) -> Result<Vec<ModEntry>, String> {
    ...
    if !dir.exists() { fs::create_dir_all(&dir).unwrap_or_default(); return Ok(vec![]); }

    let mut out = Vec::new();
    for entry in fs::read_dir(&dir).unwrap().flatten() {
        if !entry.path().is_file() { continue; }
        let filename = entry.file_name().to_string_lossy().to_string();
        if !is_resource_pack_file(&filename) { continue; }
...
pub fn toggle_resource_pack(instance_dir: String, filename: String, enable: bool) -> Result<String, String> {
    ...
    let new_file = if enable { format!("{}.zip", base) } else { format!("{}.zip.disabled", base) };
    let new_path = dir.join(&new_file);

    if current != new_path { fs::rename(&current, &new_path).unwrap(); }
    Ok(new_file)
}